...
def _run(cmd: list[str]) -> dict:
    result = subprocess.run(
        cmd, capture_output=True, cwd=REPO,
    )
    return {
...
def main() -> None:
    ...
    report_lines.append("")
    report_lines.append("> **Note**: This is an informational baseline only. No large-scale fixes required per Phase A-3 rules.")
    report_lines.append("")
    ...
    summary_line = [l for l in ruff["stdout"].split("\n") if l.startswith("Found")][0] if any(l.startswith("Found") for l in ruff["stdout"].split("\n")) else "N/A"
    ...
    try:
        ...
        report_lines.append(f"- **Confidence**: High={metrics.get('CONFIDENCE.HIGH', 0)}, Medium={metrics.get('CONFIDENCE.MEDIUM', 0)}, Low={metrics.get('CONFIDENCE.LOW', 0)}")
        for x in results[:10]:
            report_lines.append(f"  - `{x['test_id']}`: {Path(x['filename']).relative_to(REPO)}:{x['line_number']} — {x['issue_text'][:80]}")
    ...
    report_lines.append("|------|--------|--------|")
    report_lines.append(f"| ruff lint | {summary_line} | Fix with `ruff check app/ --fix` (opt-in) |")
    report_lines.append(f"| ruff format | {len(fmt_lines)} files need formatting | Fix with `ruff format app/` (opt-in) |")
    report_lines.append(f"| mypy | {len(err_lines)} errors in {len(unique_files)} files | Fix selectively (opt-in) |")
    report_lines.append(f"| bandit | {len(results) if 'results' in dir() else '?'} findings | Review high-severity items (opt-in) |")
    report_lines.append("")
    report_lines.append("---")
    report_lines.append("All baselines are informational. No code modifications required for Phase A-3.")
