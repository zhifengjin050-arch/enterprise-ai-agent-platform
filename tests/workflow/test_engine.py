...
from app.workflow_engine.models import WorkflowStatus, WorkflowDefinition, WorkflowExecution
...
class TestWorkflowEngineDefinitionCRUD:
    ...
    async def engine(self, db_engine) -> WorkflowEngine:
        ...
        for run_id, task in list(eng._active_runs.items()):
            if task and not task.done():
                task.cancel()
        eng._active_runs.clear()
        eng._paused_runs.clear()

    async def test_create_workflow(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
    ...
    async def test_get_workflow(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
    ...
    async def test_get_workflow_wrong_tenant(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
        result = await engine.get_workflow(created["id"], tenant_id="t2")
        assert result is None

    async def test_get_workflows_list(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
    ...
    async def test_get_workflows_status_filter(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
    ...
    async def test_update_workflow(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
    ...
    async def test_delete_workflow(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
    ...
    async def test_get_workflow_returns_tenant(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="tenant-42")
        assert created["tenant_id"] == "tenant-42"

    async def test_get_workflow_includes_node_count(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
...
class TestWorkflowEngineExecution:
    ...
    @pytest.fixture
    def engine(self, db_engine) -> WorkflowEngine:
        test_factory = async_sessionmaker(
            db_engine, class_=AsyncSession, expire_on_commit=False
        )
        return WorkflowEngine(session_factory=test_factory)

    async def test_execute_workflow(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
    ...
    async def test_execute_workflow_invalid_trigger(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
    ...
    async def test_get_run(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
    ...
    async def test_get_runs_for_workflow(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
        await engine.execute_workflow(created["id"], tenant_id="t1")
        runs = await engine.get_runs(workflow_id=created["id"], tenant_id="t1")
        assert len(runs) == 1

    async def test_pause_workflow(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
    ...
    async def test_resume_workflow(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_with_approval, tenant_id="t1")
        run = await engine.execute_workflow(created["id"], tenant_id="t1")
        import asyncio
        await asyncio.sleep(0.2)
    ...
    async def test_cancel_workflow(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_with_approval, tenant_id="t1")
        run = await engine.execute_workflow(created["id"], tenant_id="t1")
        import asyncio
        await asyncio.sleep(0.2)
    ...
    async def test_get_run_events(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
        run = await engine.execute_workflow(created["id"], tenant_id="t1")
        import asyncio
    ...
    async def test_tenant_isolation_runs(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
        await engine.execute_workflow(created["id"], tenant_id="t1")
...
class TestWorkflowEngineEdgeCases:
    ...
    async def test_create_with_approval_workflow(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
        result = await engine.create_workflow(sample_workflow_with_approval, tenant_id="t1")
        assert result["node_count"] == 4

    async def test_create_with_condition_workflow(self, engine: WorkflowEngine, sample_workflow_with_condition: Dict[str, Any]) -> None:
        result = await engine.create_workflow(sample_workflow_with_condition, tenant_id="t1")
        assert result["node_count"] == 5

    async def test_create_with_tool_workflow(self, engine: WorkflowEngine, sample_workflow_with_tool: Dict[str, Any]) -> None:
        result = await engine.create_workflow(sample_workflow_with_tool, tenant_id="t1")
        assert result["node_count"] == 3

    async def test_execute_twice_fails(self, engine: WorkflowEngine, sample_workflow_definition: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_definition, tenant_id="t1")
        await engine.execute_workflow(created["id"], tenant_id="t1")
        with pytest.raises(ValueError, match="already running"):
            await engine.execute_workflow(created["id"], tenant_id="t1")

    async def test_cancel_cancels_task(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_with_approval, tenant_id="t1")
    ...
    async def test_pause_saves_state(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_with_approval, tenant_id="t1")
        run = await engine.execute_workflow(created["id"], tenant_id="t1")
        import asyncio
    ...
    async def test_resume_clears_paused(self, engine: WorkflowEngine, sample_workflow_with_approval: Dict[str, Any]) -> None:
        created = await engine.create_workflow(sample_workflow_with_approval, tenant_id="t1")
        run = await engine.execute_workflow(created["id"], tenant_id="t1")
        import asyncio
        await asyncio.sleep(0.3)
